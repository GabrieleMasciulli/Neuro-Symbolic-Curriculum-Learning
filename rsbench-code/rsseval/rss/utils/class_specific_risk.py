...
It also includes a simpler per-concept risk computation for multilabel
datasets (e.g. BOIA) where concepts are independent binary attributes.
"""
...
def compute_class_specific_risk_bound(
    target_class_index, observed_partial_risk, num_classes, Sigma
):
    """
...
def get_concepts_and_labels_mnist(
    ...
    if not is_ood and dataset_name.lower() in [
        "shortmnist",
        "clipshortmnist",
        "shortmnist",
    ]:
        ...
        disallowed = torch.ones(
            label_logits.size(1), dtype=torch.bool, device=label_logits.device
        )
    ...
    predicted_concepts = predicted_concepts.reshape(
        -1
    )  # from [batch_size, 2] to [batch_size * 2]
    refactored_true_concepts = true_concepts.reshape(
        -1
    )  # from [batch_size, 2] to [batch_size * 2]
...
def compute_class_specific_risks_from_model(
    ...
    for target_class in range(num_classes):
        risk_bound = compute_class_specific_risk_bound(
            target_class, observed_partial_risk, num_classes, Sigma
        )
...
# ---------------------------------------------------------------------------
# Multilabel (BOIA-style) per-concept risk
# ---------------------------------------------------------------------------


def compute_multilabel_class_specific_risks_from_model(
    model,
    data_loader,
    num_concepts=21,
    verbose=False,
):
    """Compute per-concept error rates for a multilabel dataset (e.g. BOIA).

    For each binary concept *j* the risk is simply the mis-classification rate:

        R_j = (1 / N) * sum_i  1[ pred_j(i) != gt_j(i) ]

    This is the multilabel analogue of the class-specific risk used for MNIST
    tasks.  Because each concept is an independent binary variable the
    constrained-optimisation algorithm is not needed.

    Args:
        model: Trained model whose forward pass returns ``CS`` (sigmoid concept
               probabilities of shape ``(batch, C)``).
        data_loader: DataLoader for validation / test data.
        num_concepts: Number of concepts *C* (default 21 for BOIA).
        verbose: Whether to print progress.

    Returns:
        per_concept_risks: 1-D numpy array of shape ``(C,)`` with error rates.
        overall_concept_error: Scalar - average concept error across all
    ...
    iterator = (
        tqdm(data_loader, desc="Computing multilabel concept risks")
        if verbose
        else data_loader
    )
    ...
    with torch.no_grad():
        for data in iterator:
            ...
            # CS has sigmoid concept probabilities, shape (batch, C)
            c_probs = out_dict["CS"]
    ...
    all_pred_concepts = np.concatenate(all_pred_concepts, axis=0)  # (N, C)
    all_true_concepts = np.concatenate(all_true_concepts, axis=0)  # (N, C)

    # Per-concept error rate
    per_concept_errors = (
        (all_pred_concepts != all_true_concepts).astype(float).mean(axis=0)
    )  # (C,)
    overall_concept_error = float(per_concept_errors.mean())

    if verbose:
        for j in range(num_concepts):
            print(f"  Concept {j}: error rate = {per_concept_errors[j]:.4f}")
        print(f"  Overall concept error: {overall_concept_error:.4f}")

    return per_concept_errors, overall_concept_error
